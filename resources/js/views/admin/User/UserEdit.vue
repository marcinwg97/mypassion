<template>
    ...
    export default {
        ...
        methods:{
     loadUser:function(){
  axios.get('/api/admin/user/' + this.$route.params.id).then(res=>{
       if(res.status==200){
         this.user=res.data;
       }
     }).catch(err=>{
         console.log(err)
            ...
            editUser(id){
                axios.post('/api/admin/user/' + id, {name: this.user.name, email: this.user.email, password: this.user.password})
                .then((response)=>{
